...
<div class="col-xs-12 col-sm-12 col-md-12 portfolio">
 ...
 @if(!empty($job->appreoved_date)){{ Format::displayDate($job->appreoved_date  )  }} @else&nbsp;@endif
										...
										<div class="col-xs-6 col-sm-4 col-md-4">
											<h6 class ="marginh6 tit-minheight">Role match status</h6>
										</div>
										<div class="col-xs-6 col-sm-8 col-md-6">
											<h6 class ="marginh6 tit-minheight">
											<?php $jobmatchstatus =Config::get('ins.jobmatchstatus');
									...
									<div class="col-xs-4 col-sm-4 col-md-2 hidden1">
 										<?php  
										 	$potential_match = Config::get('ins.match_history');
										 ?>

										 @if(!empty($job->deadline_date) and $job->deadline_date < date('Y-m-d'))
					                         <span class="applimarg" style=""><b>Expired </b></span>
					                    
										 @else 



												 @if(!empty($job->jobmatchstatus) and in_array($job->jobmatchstatus, $potential_match) ) 
												 ...
												 @else 
														...
														@if(!empty($job->job_eoi_rejected)) 
															<span class="applimarg" style=""><b>Unmatched Role</b></span>
														@else
															@if(empty($job->job_eoi)) 
																	@if(!empty($job->isjobeoiapply)) 
																		...
																		@elseif(!empty($job->job_draft)) 
								                                             <span class=" "  style="color:red; float:right" >Application Draft</span>
								                                     	@else
																			<button id="applybutton1" class="btn btn-primary apply1 ">Apply</button>
																		@endif
																	@else
																		<button id="iminterested" class="btn btn-primary apply1 ">I'm interested</button>
																	 	<button  type="button"  class="btn btn-primary reject1 " data-toggle="modal" data-target="#myModal">I'm not a suitable match</button>
																	@endif

															@else
																@if(!empty($job->job_eoi)  ) 
																	<span class="applimarg" style=""><b>Interest Registered</b></span>
																@else
																	@if(!empty($job->job_apply)) 
																			<span class="applimarg" style=""><b>You already applied</b></span>
																	@else
