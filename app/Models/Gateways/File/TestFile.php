...
/**
 * External File Gateway Implmentation for Test Files
 */
class TestFile implements ExternalFileGateway
{
    ...
    public function download($source, $destination, $options = array())
    {
        ...
        $proxy = null;
        if (!empty($destination) && !empty($content)) {
            file_put_contents($destination, $content);
            $proxy = new FileProxy($destination, 'file');
        } else {
            $proxy = new FileProxy($content, 'data');
        }
        return $proxy;
    }
