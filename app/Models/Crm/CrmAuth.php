...
class CrmAuth {
	...
	public function GetHeaderOnline($username, $password, $url) {
		echo "Curl GetHeaderOnline method executing...\n";
		$url .= (substr($url, - 1) == '/' ? '' : '/');
		...
		$xml .= "<o:Username><![CDATA[".$username."]]></o:Username>";
		$xml .= "<o:Password><![CDATA[".$password."]]></o:Password>";
		$xml .= "</o:UsernameToken>";
		$xml .= "</o:Security>";
		$xml .= "</s:Header>";
		$xml .= "<s:Body>";
		...
		echo "Curl executing for authentication...\n";
		$ch = curl_init();
		...
		$AllElements = $responsedom->getElementsByTagName('*');
	    foreach($AllElements as $OneElement){
	    	if(($OneElement->tagName) == 'Reason'){
	    		$internalerror = $responsedom->getElementsbyTagName("Reason");
				echo "Curl error: ".$internalerror->item(0)->textContent."\n";
	    	}//if reason element exists
	    }//foreach
	...
	function GetADFS($url) {
		...
		curl_setopt(
			$ch,
			CURLOPT_URL,
			$url . "XrmServices/2011/Organization.svc?wsdl=wsdl0"
		);
		curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, 1 );
		curl_setopt ( $ch, CURLOPT_TIMEOUT, 60 );
		curl_setopt ( $ch, CURLOPT_SSL_VERIFYPEER, false );
